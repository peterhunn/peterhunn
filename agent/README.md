...
**Read is universal** across all four (each requires that user's explicit
OAuth grant). **Send is per-account** with the policies above.
...
Merged config in `email-family-ea.example.jsonc`. Summary:
...
Shweta needs her own bot so she approves her own drafts — nothing routes
through Peter for her decisions.
...
Copy `CONSENT.md` from this repo, fill it in with Peter, Shweta, and
whoever consents for `office@`, and keep the signed copy in
`~/.openclaw/workspace/CONSENT.md`. May reads it as part of her
