...
Product and architecture spec lives alongside this file in
[`docs/`](./README.md).
...
- [`20-architecture.md`](./20-architecture.md) — the invariants
  (tenancy, provenance, policy authority, approval queue, etc.)
  that every module assumes.
- [`40-security.md`](./40-security.md) — what's real today vs the
  gaps that must close before a paying customer. Threat model.
- [`60-contributing.md`](./60-contributing.md) — recipes for the
...
verification, and update flow live in [`50-deployment.md`](./50-deployment.md).
...
- Agent runtime — Orchestrator + typed Agent/Tool contracts + task
  ...
  - `travel` — handles `travel.trip.plan` (T3
    ...
    bench from 24-model-routing.md runs end-to-end on a fresh clone.
...
- Credentials store — a first-class `credentials` table + repo +
  ...
  the bulk upgrade. See 40-security.md and 50-deployment.md for threat
  model and rotation.
