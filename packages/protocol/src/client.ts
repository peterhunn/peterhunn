...
/**
 * A fetch-compatible client that automatically traverses the x430 protocol.
 *
 * On a 430 with X-430-Requirements, it establishes the agreement and retries
 * with X-430-Contract. On a 402 with contractRequired in the body, it handles
 * the x430 gate before the caller handles x402 payment.
 *
 * Usage:
...
export class ContractClient {
  ...
  async fetch(url: string, init?: RequestInit): Promise<Response> {
    ...
    if (cached) headers.set("X-430-Contract", cached);

    const response = await fetch(url, { ...init, headers });

    // x430 gate on 430
    if (response.status === 430) {
      const reqHeader = response.headers.get("X-430-Requirements");
      ...
      retryHeaders.set("X-430-Contract", token);
      return fetch(url, { ...init, headers: retryHeaders });
    }

    // x402 + x430 combined gate on 402
    if (response.status === 402) {
      ...
      if (body.contractRequired) {
        ...
        // Return the 402 with the x430 requirement satisfied — the caller
  ...
  async establishAgreement(requirements: ContractRequirements): Promise<string> {
    ...
    while (round < this.maxRounds) {
      ...
      if (!res.ok) {
        throw new Error(`x430 accept failed: ${res.status} ${await res.text()}`);
      }
    ...
    throw new Error(`x430 negotiation exceeded ${this.maxRounds} rounds`);
  }
  ...
  attachToken(resource: string, headers: Headers): void {
    ...
    if (token) headers.set("X-430-Contract", token);
  }
