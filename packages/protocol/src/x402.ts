...
/**
 * Build a combined x402 + x430 402 response body.
 *
 ...
 * contractRequired field; x430-aware clients process both gates.
 */
 ...
 * embedded x430 ContractRequirements.
 */
...
/**
 * Additional response headers to include alongside a 402 that carries x430 requirements.
 *
 * Allows clients that check headers (not the body) to discover the contract gate.
 */
export function x430ExtensionHeaders(
  contractRequired: ContractRequirements,
): Record<string, string> {
  return {
    "X-430-Requirements": b64encode(JSON.stringify(contractRequired)),
  };
}

/**
 * Given a fetch Response that returned 402, extract any embedded x430
 * ContractRequirements from the body or X-430-Requirements header.
 *
 * Returns undefined if this is a plain x402 response with no x430 layer.
 */
export async function extractContractRequirements(
  response: Response,
): Promise<ContractRequirements | undefined> {
  const header = response.headers.get("X-430-Requirements");
  if (header) {
    try {
